...
sysargmap2="/dev/null"
sysargmap2_h="_SYS_SYSARGMAP2_H_"
...
' < $1 | awk "
	BEGIN {
		...
		sysargmap2 = \"$sysargmap2\"
		sysargmap2_h = \"$sysargmap2_h\"
		...
		printf "#ifndef %s\n", sysargmap2_h > sysargmap2
		printf "#define\t%s\n\n", sysargmap2_h > sysargmap2
		printf "int %s[] = {\n",ptrmaskname > sysargmap2
	...
	type("STD") || type("NODEF") || type("NOARGS") || type("NOPROTO") \
		...
		if (argc != 0 && !flag("NOARGS") && !flag("NODEF")) {
			...
			printf(" [%s%s] = (0x0", syscallprefix, funcalias) > sysargmap2
			for (i = 1; i <= argc; i++)
