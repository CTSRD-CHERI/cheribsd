...
caprevoke_fini(struct caprevoke_syscall_info * __capability crsi,
    int res, struct caprevoke_stats *crst, struct caprevoke_info *cri)
{
	int res2 = 0;
	int res3 = 0;
...
#endif
			res2 = 0;
		res3 = copyout(cri, &crsi->info, sizeof(*cri));
	}
	if (res != 0)
		return res;
	if (res2 != 0)
...
fast_out:
			...
			return caprevoke_fini(crsi, ires, crstp, &cri);
		}
	...
	vm_caprevoke_publish(info_page, &cri);
	wmb();
...
skip_last_pass:
	...
	return caprevoke_fini(crsi, 0, crstp, &cri);
}
