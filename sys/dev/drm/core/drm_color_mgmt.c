			     ...
			     void *data, struct drm_file *file_priv)
{
	...
	if (!SV_CURPROC_FLAG(SV_CHERI)) {
		...
		crtc_lut->red = (uintptr_t)__USER_CAP(crtc_lut64->red, size);
		crtc_lut->green = (uintptr_t)__USER_CAP(crtc_lut64->green,
		    size);
		crtc_lut->blue = (uintptr_t)__USER_CAP(crtc_lut64->blue, size);
	}
#endif
	...
	r_base = crtc->gamma_store;
	if (copy_from_user(r_base, (void __user *)crtc_lut->red, size)) {
		ret = -EFAULT;
		goto out;
	}

	g_base = r_base + size;
	if (copy_from_user(g_base, (void __user *)crtc_lut->green, size)){
		ret = -EFAULT;
		goto out;
	}

	b_base = g_base + size;
	if (copy_from_user(b_base, (void __user *)crtc_lut->blue, size)) {
		ret = -EFAULT;
		goto out;
	}
			     ...
			     void *data, struct drm_file *file_priv)
{
	...
	size = crtc_lut->gamma_size * (sizeof(uint16_t));
	r_base = crtc->gamma_store;
	if (copy_to_user((void __user *)crtc_lut->red, r_base, size)) {
		ret = -EFAULT;
		goto out;
	}

	g_base = r_base + size;
	if (copy_to_user((void __user *)crtc_lut->green, g_base, size)) {
		ret = -EFAULT;
		goto out;
	}

	b_base = g_base + size;
	if (copy_to_user((void __user *)crtc_lut->blue, b_base, size)) {
		ret = -EFAULT;
		goto out;
	}
out:
