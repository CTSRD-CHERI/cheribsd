...
systrace_args(int sysnum, void *params, uintcap_t *uarg, int *n_args)
{
	...
	case 7: {
		...
		uarg[a++] = (intcap_t)p->rusage; /* struct rusage32 * */
		*n_args = 4;
		break;
	}
	/* link */
	...
	case 27: {
		...
		iarg[a++] = p->s; /* int */
		uarg[a++] = (intcap_t)p->msg; /* struct msghdr32 * */
		iarg[a++] = p->flags; /* int */
		*n_args = 3;
		break;
	}
	...
	case 28: {
		...
		iarg[a++] = p->s; /* int */
		uarg[a++] = (intcap_t)p->msg; /* const struct msghdr32 * */
		iarg[a++] = p->flags; /* int */
		*n_args = 3;
		break;
	}
	...
	case 53: {
		...
		uarg[a++] = (intcap_t)p->ss; /* const struct sigaltstack32 * */
		uarg[a++] = (intcap_t)p->oss; /* struct sigaltstack32 * */
		*n_args = 2;
		break;
	}
	/* freebsd32_ioctl */
	...
	case 59: {
		...
		uarg[a++] = (intcap_t)p->fname; /* const char * */
		uarg[a++] = (intcap_t)p->argv; /* uint32_t * */
		uarg[a++] = (intcap_t)p->envv; /* uint32_t * */
		*n_args = 3;
		break;
	}
	/* umask */
	...
	case 83: {
		...
		iarg[a++] = p->which; /* int */
		uarg[a++] = (intcap_t)p->itv; /* const struct itimerval32 * */
		uarg[a++] = (intcap_t)p->oitv; /* struct itimerval32 * */
		*n_args = 3;
		break;
	}
	/* swapon */
	...
	case 86: {
		...
		iarg[a++] = p->which; /* int */
		uarg[a++] = (intcap_t)p->itv; /* struct itimerval32 * */
		*n_args = 2;
		break;
	}
	/* getdtablesize */
	...
	case 93: {
		...
		uarg[a++] = (intcap_t)p->tv; /* struct timeval32 * */
		*n_args = 5;
		break;
	}
	/* fsync */
	...
	case 116: {
		...
		uarg[a++] = (intcap_t)p->tp; /* struct timeval32 * */
	...
	case 120: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->iovp; /* const struct iovec32 * */
	...
	case 122: {
		...
		uarg[a++] = (intcap_t)p->tv; /* const struct timeval32 * */
	...
	case 138: {
		...
		uarg[a++] = (intcap_t)p->path; /* const char * */
		uarg[a++] = (intcap_t)p->tptr; /* const struct timeval32 * */
		*n_args = 2;
		break;
	}
	/* freebsd32_adjtime */
	case 140: {
		...
		uarg[a++] = (intcap_t)p->delta; /* const struct timeval32 * */
		uarg[a++] = (intcap_t)p->olddelta; /* struct timeval32 * */
		*n_args = 2;
		break;
	}
	/* setsid */
	...
	case 176: {
		...
		uarg[a++] = (intcap_t)p->tp; /* struct timex32 * */
		*n_args = 1;
		break;
	}
	/* setgid */
	...
	case 202: {
		...
		uarg[a++] = (intcap_t)p->oldlenp; /* uint32_t * */
	...
	case 232: {
		...
		uarg[a++] = (intcap_t)p->tp; /* struct timespec32 * */
		*n_args = 2;
		break;
	}
	...
	case 233: {
		...
		iarg[a++] = p->clock_id; /* clockid_t */
		uarg[a++] = (intcap_t)p->tp; /* const struct timespec32 * */
		*n_args = 2;
		break;
	}
	...
	case 235: {
		...
		iarg[a++] = p->clock_id; /* clockid_t */
		uarg[a++] = (intcap_t)p->evp; /* struct sigevent32 * */
	...
	case 237: {
		...
		iarg[a++] = p->timerid; /* int */
		iarg[a++] = p->flags; /* int */
		uarg[a++] = (intcap_t)p->value; /* const struct itimerspec32 * */
		uarg[a++] = (intcap_t)p->ovalue; /* struct itimerspec32 * */
		*n_args = 4;
		break;
	}
	...
	case 238: {
		...
		iarg[a++] = p->timerid; /* int */
		uarg[a++] = (intcap_t)p->value; /* struct itimerspec32 * */
		*n_args = 2;
		break;
	}
	/* ktimer_getoverrun */
	...
	case 240: {
		...
		uarg[a++] = (intcap_t)p->rqtp; /* const struct timespec32 * */
		uarg[a++] = (intcap_t)p->rmtp; /* struct timespec32 * */
		*n_args = 2;
		break;
	}
	/* ffclock_getcounter */
	...
	case 242: {
		...
		uarg[a++] = (intcap_t)p->cest; /* struct ffclock_estimate32 * */
		*n_args = 1;
		break;
	}
	...
	case 255: {
		...
		uarg[a++] = (intcap_t)p->aiocbp; /* struct aiocb32 * */
		*n_args = 1;
		break;
	}
	/* freebsd32_aio_write */
	...
	case 257: {
		...
		iarg[a++] = p->mode; /* int */
		uarg[a++] = (intcap_t)p->acb_list; /* uint32_t * */
		iarg[a++] = p->nent; /* int */
		uarg[a++] = (intcap_t)p->sig; /* struct sigevent32 * */
		*n_args = 4;
		break;
	}
	/* kbounce */
	...
	case 289: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->iovp; /* struct iovec32 * */
		uarg[a++] = p->iovcnt; /* u_int */
	...
	case 301: {
		...
		iarg[a++] = p->modid; /* int */
		uarg[a++] = (intcap_t)p->stat; /* struct module_stat32 * */
		*n_args = 2;
		break;
	}
	/* modfnext */
	...
	case 308: {
		...
		iarg[a++] = p->fileid; /* int */
		uarg[a++] = (intcap_t)p->stat; /* struct kld_file_stat32 * */
		*n_args = 2;
		break;
	}
	/* kldfirstmod */
	...
	case 315: {
		...
		uarg[a++] = (intcap_t)p->aiocbp; /* uint32_t * */
		iarg[a++] = p->nent; /* int */
		uarg[a++] = (intcap_t)p->timeout; /* const struct timespec32 * */
		*n_args = 3;
		break;
	}
	...
	case 334: {
		...
		iarg[a++] = p->pid; /* pid_t */
		uarg[a++] = (intcap_t)p->interval; /* struct timespec32 * */
		*n_args = 2;
		break;
	}
	/* utrace */
	...
	case 338: {
		...
		uarg[a++] = (intcap_t)p->jailp; /* struct jail32 * */
		*n_args = 1;
		break;
	}
	/* sigprocmask */
	...
	case 345: {
		...
		uarg[a++] = (intcap_t)p->set; /* const sigset_t * */
		uarg[a++] = (intcap_t)p->info; /* struct __siginfo32 * */
		uarg[a++] = (intcap_t)p->timeout; /* const struct timespec32 * */
		*n_args = 3;
		break;
	}
	...
	case 359: {
		...
		uarg[a++] = (intcap_t)p->aiocbp; /* uint32_t * */
		uarg[a++] = (intcap_t)p->timeout; /* struct timespec32 * */
		*n_args = 2;
		break;
	}
	/* getresuid */
	...
	case 393: {
		...
		uarg[a++] = p->offset2; /* uint32_t */
		uarg[a++] = p->nbytes; /* size_t */
		uarg[a++] = (intcap_t)p->hdtr; /* struct sf_hdtr32 * */
	...
	case 416: {
		...
		iarg[a++] = p->sig; /* int */
		uarg[a++] = (intcap_t)p->act; /* const struct sigaction32 * */
		uarg[a++] = (intcap_t)p->oact; /* struct sigaction32 * */
		*n_args = 3;
		break;
	}
	/* freebsd32_sigreturn */
	case 417: {
		...
		uarg[a++] = (intcap_t)p->sigcntxp; /* const struct __ucontext32 * */
		*n_args = 1;
		break;
	}
	...
	case 421: {
		...
		uarg[a++] = (intcap_t)p->ucp; /* struct __ucontext32 * */
		*n_args = 1;
		break;
	}
	...
	case 422: {
		...
		uarg[a++] = (intcap_t)p->ucp; /* const struct __ucontext32 * */
		*n_args = 1;
		break;
	}
	...
	case 423: {
		...
		uarg[a++] = (intcap_t)p->oucp; /* struct __ucontext32 * */
		uarg[a++] = (intcap_t)p->ucp; /* const struct __ucontext32 * */
		*n_args = 2;
		break;
	}
	...
	case 431: {
		...
		uarg[a++] = (intcap_t)p->state; /* int32_t * */
		*n_args = 1;
		break;
	}
	/* thr_self */
	...
	case 441: {
		...
		iarg[a++] = p->id; /* int32_t */
		uarg[a++] = (intcap_t)p->abstime; /* const struct timespec32 * */
		*n_args = 2;
		break;
	}
	...
	case 455: {
		...
		uarg[a++] = (intcap_t)p->param; /* struct thr_param32 * */
	...
	case 457: {
		...
		iarg[a++] = p->flags; /* int */
		iarg[a++] = p->mode; /* mode_t */
		uarg[a++] = (intcap_t)p->attr; /* const struct mq_attr32 * */
		*n_args = 4;
		break;
	}
	...
	case 458: {
		...
		iarg[a++] = p->mqd; /* int */
		uarg[a++] = (intcap_t)p->attr; /* const struct mq_attr32 * */
		uarg[a++] = (intcap_t)p->oattr; /* struct mq_attr32 * */
		*n_args = 3;
		break;
	}
	...
	case 459: {
		...
		uarg[a++] = (intcap_t)p->abs_timeout; /* const struct timespec32 * */
		*n_args = 5;
		break;
	}
	...
	case 461: {
		...
		iarg[a++] = p->mqd; /* int */
		uarg[a++] = (intcap_t)p->sigev; /* const struct sigevent32 * */
		*n_args = 2;
		break;
	}
	/* kmq_unlink */
	...
	case 463: {
		...
		iarg[a++] = p->nargs; /* int */
		uarg[a++] = (intcap_t)p->args; /* uint32_t * */
		*n_args = 3;
		break;
	}
	/* thr_set_name */
	...
	case 473: {
		...
		iarg[a++] = p->sd; /* int */
		uarg[a++] = (intcap_t)p->iov; /* struct iovec32 * */
		iarg[a++] = p->iovlen; /* int */
		uarg[a++] = (intcap_t)p->to; /* const struct sockaddr * */
		iarg[a++] = p->tolen; /* __socklen_t */
		uarg[a++] = (intcap_t)p->sinfo; /* struct sctp_sndrcvinfo * */
	...
	case 494: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->path; /* const char * */
		uarg[a++] = (intcap_t)p->times; /* const struct timeval32 * */
		*n_args = 3;
		break;
	}
	/* linkat */
	...
	case 510: {
		...
		iarg[a++] = p->semnum; /* int */
		iarg[a++] = p->cmd; /* int */
		uarg[a++] = (intcap_t)p->arg; /* union semun32 * */
		*n_args = 4;
		break;
	}
	/* freebsd32_msgctl */
	case 511: {
		...
		iarg[a++] = p->msqid; /* int */
		iarg[a++] = p->cmd; /* int */
		uarg[a++] = (intcap_t)p->buf; /* struct msqid_ds32 * */
		*n_args = 3;
		break;
	}
	/* freebsd32_shmctl */
	case 512: {
		...
		iarg[a++] = p->shmid; /* int */
		iarg[a++] = p->cmd; /* int */
		uarg[a++] = (intcap_t)p->buf; /* struct shmid_ds32 * */
		*n_args = 3;
		break;
	}
	/* lpathconf */
	...
	case 522: {
		...
		uarg[a++] = (intcap_t)p->ou; /* fd_set * */
		uarg[a++] = (intcap_t)p->ex; /* fd_set * */
		uarg[a++] = (intcap_t)p->ts; /* const struct timespec32 * */
	...
	case 532: {
		...
		uarg[a++] = (intcap_t)p->status; /* int * */
		iarg[a++] = p->options; /* int */
		uarg[a++] = (intcap_t)p->wrusage; /* struct __wrusage32 * */
		uarg[a++] = (intcap_t)p->info; /* struct __siginfo32 * */
		*n_args = 8;
		break;
	}
	...
	case 534: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->cmds; /* const uint32_t * */
	...
	case 535: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->cmds; /* uint32_t * */
	...
	case 546: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->times; /* const struct timespec32 * */
		*n_args = 2;
		break;
	}
	/* freebsd32_utimensat */
	...
	case 551: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->sb; /* struct stat32 * */
		*n_args = 2;
		break;
	}
	/* freebsd32_fstatat */
	case 552: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->path; /* const char * */
		uarg[a++] = (intcap_t)p->buf; /* struct stat32 * */
		iarg[a++] = p->flag; /* int */
		*n_args = 4;
		break;
	}
	...
	case 560: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->changelist; /* const struct kevent32 * */
		iarg[a++] = p->nchanges; /* int */
		uarg[a++] = (intcap_t)p->eventlist; /* struct kevent32 * */
	...
	case 586: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intcap_t)p->curr_value; /* struct itimerspec32 * */
		*n_args = 2;
		break;
	}
	...
	case 587: {
		...
		iarg[a++] = p->fd; /* int */
		iarg[a++] = p->flags; /* int */
		uarg[a++] = (intcap_t)p->new_value; /* const struct itimerspec32 * */
		uarg[a++] = (intcap_t)p->old_value; /* struct itimerspec32 * */
		*n_args = 4;
		break;
	}
	/* kcmp */
