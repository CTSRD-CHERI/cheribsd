...
$code.=<<___	if ($flavour =~ /64/);
	...
	str	$tmpinp,[csp,#-80]!
	str	$constnumx,[csp,#64]
#else
	stp	$constnumx,$tmpinp,[sp,#-64]!
#endif
	stp	$tailcnt,$midnumx,[PTRN(sp),#48]
	stp	$ivd10,$ivd20,[PTRN(sp),#32]
	stp	$ivd30,$ivd40,[PTRN(sp),#16]
...
.Lxts_abort:
	ldp	$tailcnt,$midnumx,[PTRN(sp),#48]
	ldp	$ivd10,$ivd20,[PTRN(sp),#32]
	ldp	$ivd30,$ivd40,[PTRN(sp),#16]
#ifdef __CHERI_PURE_CAPABILITY__
	ldr	$constnumx,[csp,#64]
	ldr	$tmpinp,[csp],#80
#else
