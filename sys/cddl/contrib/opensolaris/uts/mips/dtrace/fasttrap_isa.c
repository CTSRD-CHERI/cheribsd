...
{
	ft_printf("Installing tracepoint at %lx\n", (uint64_t)tp->ftt_pc);
...
{
	ft_printf("removing tracepoint from %lx\n", (uint64_t)tp->ftt_pc);
	uint32_t instr;
	...
	if (instr != FASTTRAP_INSTR) {
		ft_printf(
		    "Error removing tracepoint. Precedent: 0x%x\n", instr);
		return (-1);
	}
...
fasttrap_clear_single_step(proc_t *p, fasttrap_tracepoint_t *tp)
{
	...
	ft_printf("Single step cleaned! 0x%x written to 0x%lx\n",
