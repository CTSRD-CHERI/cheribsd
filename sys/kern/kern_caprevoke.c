...
static int
caprevoke_fini(struct thread *td, struct caprevoke_stats * __capability statout,
...
#error "Can't get stack frame on this architecture"
#endif
		vm_caprevoke_one(vmcrc, 0, (vm_offset_t)sp);
	}
	...
	stats->epoch_fini = vmcrc->map->vm_caprev_st >> CAPREVST_EPOCH_SHIFT;
}
    ...
    struct caprevoke_stats * __capability statout)
{
	...
	if (res != KERN_SUCCESS) {
		vmspace_free(vm);
		return caprevoke_fini(
		    td, statout, vm_mmap_to_errno(res), &stat);
	}
	...
	if (myst == CAPREVST_LAST_PASS) {
		...
		res = vm_caprevoke(&vmcrc,
		    ((entryst == CAPREVST_INIT_DONE) ?
				  VM_CAPREVOKE_INCREMENTAL :
				  0) |
			VM_CAPREVOKE_LAST_INIT | VM_CAPREVOKE_PMAP_SYNC |
		...
		if ((flags & CAPREVOKE_LAST_NO_LATE) == 0) {
			 ...
			 */
			res = vm_caprevoke(&vmcrc,
			    VM_CAPREVOKE_INCREMENTAL | VM_CAPREVOKE_LAST_FINI);
		}
	}
