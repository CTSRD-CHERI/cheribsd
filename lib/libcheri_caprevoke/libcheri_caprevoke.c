 ...
 */
static inline void
caprev_shadow_nomap_common_pfx(uint64_t *__capability sb, vaddr_t ob,
    size_t len, ptrdiff_t *fwo, ptrdiff_t *lwo, uint64_t *__capability *fw)
{
 ...
 */
int
caprev_shadow_nomap_set_len(uint64_t *__capability sb, vaddr_t ob, size_t len,
    void *__capability user_obj)
{
	ptrdiff_t fwo, lwo;
	uint64_t *__capability fw;

	caprev_shadow_nomap_common_pfx(sb, ob, len, &fwo, &lwo, &fw);
	...
	if (lwo != fwo) {
		...
		uint64_t *__capability lw =
		    cheri_setaddress(sb, VM_CAPREVOKE_BM_MEM_NOMAP + lwo);
 ...
 */
int
caprev_shadow_nomap_set(uint64_t *__capability sb, void *__capability priv_obj,
    void *__capability user_obj)
{
	return caprev_shadow_nomap_set_len(sb, (__cheri_addr vaddr_t)priv_obj,
	    cheri_getlen(priv_obj), user_obj);
}
 ...
 */
void
caprev_shadow_nomap_clear_len(uint64_t *__capability sb, vaddr_t ob, size_t len)
{
	ptrdiff_t fwo, lwo;
	uint64_t *__capability fw;
...
{
	return caprev_shadow_nomap_clear_len(
	    sb, (__cheri_addr vaddr_t)obj, cheri_getlen(obj));
}
...
caprev_shadow_nomap_set_raw(
    uint64_t *__capability sb, vaddr_t heap_start, vaddr_t heap_end)
{
	ptrdiff_t fwo, lwo;
	uint64_t *__capability fw;
